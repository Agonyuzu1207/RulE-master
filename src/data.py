...
#规则格式： [rule_head， rule_body_list].（均为id）
class RuleDataset(Dataset):
    def __init__(self, num_relations, input, negative_sample_size, mode='batch'):#关系数46、规则文件名，负样本数128
        self.rules = list()
        self.num_relations = num_relations
        # self.ending_idx = num_relations
        self.padding_idx = num_relations * 2 #92
        self.negative_sample_size = negative_sample_size#128
        self.mode = mode
        idx = 0
        ...
        with open(input, 'r') as fi:
            for line in fi:
                ...
                idx += 1
                formatted_rule = [rule, self.padding_idx]#格式化规则[idx,规则头，规则体，92]
                self.rules.append(formatted_rule)#加入规则集
...
def Iterator(dataloader):
    while True:
        for data in dataloader:
            yield data#send是发送一个参数给res的，带yield的函数是一个生成器，而不是一个函数了
