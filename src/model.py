...
class RulE(torch.nn.Module):
    def __init__(self, graph, p_norm, mlp_rule_dim, gamma_fact, gamma_rule, hidden_dim, device):
        ...
        # nn.init.ones_(
        #     tensor=self.relation_embedding.weight其随机初始化方式是自带标准正态分布
        # )
        nn.init.uniform_(#从均匀分布U(a, b)中生成值，填充输入的张量或变量
            tensor=self.relation_embedding.weight, #其随机初始化方式是自带标准正态分布
            a=-self.embedding_range_fact.item(), 
            b=self.embedding_range_fact.item()
        )
    ...
    def set_rules(self, input):#输入提取的规则
        ...
        # rule_body's length
        self.max_length = max([len(rule[2:]) for rule in input])#4

        # self.rule_dim = self.hidden_dim * self.max_length
        self.rule_dim = self.hidden_dim #2000
        ...
        self.mlp_feature = nn.Parameter(torch.zeros(self.num_rules, self.mlp_rule_dim))#18400,100
    ...
    def compute_ruleE(self, sample, mode='single'):
        ...
        if mode == 'single':#计算positive_rule_score
            rule, mask = sample
           
            score, rule_emb = self.add_ruleE(rule.unsqueeze(1), mask)

        elif mode == 'batch':# 计算negative_rule_score
